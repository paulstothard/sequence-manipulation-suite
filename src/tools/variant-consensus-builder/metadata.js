...
export const variantConsensusMetadata={
  ...
  outputType:'Consensus FASTA, linear DNA sequence viewer, variant audit table, coordinate map table, variant overview map, or report',
  ...
  workflow:{inputs:[{id:'input',kind:'text',mediaType:'text/plain'}],outputs:[
    ...
    {id:'viewer',kind:'viewer',viewerType:'dna-sequence-viewer',label:'Consensus sequence viewer'},
    ...
    {id:'map',kind:'text',mediaType:'image/svg+xml',label:'Variant overview map'},
  ...
  options:[
    ...
    {type:'group',label:'Limits',id:'advancedLimits',collapsible:true,collapsed:true,options:[
      ...
      {id:'maxInputCharacters',type:'limit-value',label:'Loaded FASTA or VCF input',value:`${CONSENSUS_LIMITS.characters.toLocaleString('en-US')} characters per source`,detail:'Applies to paste/upload mode.'},
      {id:'maxReferenceBases',type:'limit-value',label:'Loaded reference or indexed region',value:`${CONSENSUS_LIMITS.bases.toLocaleString('en-US')} bases`},
      {id:'maxOutputBases',type:'limit-value',label:'Constructed consensus',value:`${CONSENSUS_LIMITS.outputBases.toLocaleString('en-US')} bases`},
      {id:'maxSamplesAndContigs',type:'limit-value',label:'VCF samples / reference contigs',value:`${CONSENSUS_LIMITS.samples.toLocaleString('en-US')} / ${CONSENSUS_LIMITS.contigs.toLocaleString('en-US')}`},
      {id:'maxIdentifierLength',type:'limit-value',label:'Sample or contig ID',value:`${CONSENSUS_LIMITS.idLength} characters`},
      {id:'maxPhaseBlocks',type:'limit-value',label:'Phase blocks per contig',value:CONSENSUS_LIMITS.phaseBlocks},
      {id:'maxOutputRecords',type:'limit-value',label:'Output sequences',value:CONSENSUS_LIMITS.outputRecords},
      {id:'maxTableCharacters',type:'limit-value',label:'Audit / coordinate table',value:`${CONSENSUS_TABLE_CHARACTERS.toLocaleString('en-US')} output characters`},
      {id:'consensusViewerLimit',type:'limit-value',label:'Linear viewer',value:`${CONSENSUS_VIEWER_MAX_RECORDS} output sequences`},
      {id:'consensusMapLimit',type:'limit-value',label:'Static overview map',value:'12 output sequences and 240 audit rows'},

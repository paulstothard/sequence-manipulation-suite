...
export function sideLegendLayout(width, labels, { left = 86, gap = 22, rightPadding = 20 } = {}) {
  const maxWidth = Math.min(210, Math.max(116, Math.floor(width * 0.25)), Math.max(100, width - left - gap - rightPadding - 220));
  const labelWidth = Math.max(0, ...labels.map((label) => estimatedTextWidth(label)));
...
export function fitSideLegendLabel(value, maxWidth) {
  const text = String(value ?? "");
  if (estimatedTextWidth(text) <= maxWidth) return text;
  let fitted = "";
  for (const character of text) {
    if (estimatedTextWidth(`${fitted}${character}…`) > maxWidth) break;
    fitted += character;
  }

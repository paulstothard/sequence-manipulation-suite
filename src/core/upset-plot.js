...
export function makeUpsetStyleSvg({
  ...
  const setBars = setLabels.map((label, index) => {
    ...
    return `<g data-sms3-inspection-mark="set-size"><title>${escapeXml(`${label}: ${size.toLocaleString()} distinct item${size === 1 ? "" : "s"}; ${setSizeScale.mode} bar scale`)}</title><rect class="set-size-bar" data-count="${size}" data-scale="${setSizeScale.mode}" x="${x - 5}" y="${y}" width="10" height="${barHeight}" rx="2" fill="${SMS3_PLOT_THEME.textMuted}"></rect><text class="set-count" x="${x}" y="${Math.max(12, y - 6)}" text-anchor="middle">${setCountLabels[index]}</text></g>`;
  }).join("");
